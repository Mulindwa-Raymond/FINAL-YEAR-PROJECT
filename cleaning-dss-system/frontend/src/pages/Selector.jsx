...
// ======================= HERO SECTION (Reduced spacing, smaller image) =======================
const HeroSection = () => {
    ...
    <section className="relative pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-6">
        <div className="relative bg-white rounded-[40px] border border-slate-200 overflow-hidden shadow-2xl shadow-blue-900/5">
          <div className="absolute top-0 left-0 w-32 h-32 border-t-2 border-l-2 border-cyan-500/20 rounded-tl-[40px]"></div>
          
          <div className="grid lg:grid-cols-12 items-stretch min-h-[500px]">
            {/* Left Content */}
            <div className="lg:col-span-5 p-10 lg:p-16 flex flex-col justify-center border-r border-slate-100 relative z-10 bg-white">
              <div className="inline-flex items-center gap-2 mb-8 px-3 py-1 bg-cyan-50 border border-cyan-100 text-cyan-700 text-[10px] font-mono font-black tracking-widest uppercase rounded-full w-fit">
                <Terminal size={12} /> System Status: Optimal
              </div>
              
              <h1 className="text-4xl lg:text-5xl font-black leading-tight mb-8 text-slate-900 uppercase tracking-tighter">
                Smart Hygiene <br />
              ...
              <p className="text-slate-500 text-sm leading-relaxed mb-10 max-w-sm font-medium">
              ...
              <div className="flex flex-wrap gap-4 mb-10">
                <button className="group px-8 py-5 bg-slate-900 rounded-2xl font-black text-white text-xs uppercase tracking-widest transition-all hover:bg-cyan-600 hover:scale-[1.02] active:scale-95 shadow-xl shadow-slate-900/10">
                  <span className="flex items-center gap-2">Initialize Match <ChevronRight size={16} /></span>
                </button>
            ...
            {/* Right Carousel - Fixed height 380px */}
            <div className="lg:col-span-7 relative bg-slate-50">
              <div className={`absolute inset-0 transition-all duration-700 ${fade ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}>
                <img 
                ...
                <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                  <div className="bg-white/40 backdrop-blur-md px-4 py-2 rounded-full border border-white/50 text-[10px] font-mono font-bold text-cyan-800">
                  ...
                  <div className="flex gap-2 pb-2">
                    {heroImages.map((_, idx) => (
                      <div key={idx} className={`h-1.5 rounded-full transition-all duration-500 ${idx === currentIndex ? 'w-8 bg-cyan-600' : 'w-2 bg-slate-300'}`} />
                    ))}
                  </div>
                </div>
              </div>
